...
The HTTP Platform API is primarily read-only. Authentication, classroom operations, recording control, and broader remote command execution remain part of the longer-term roadmap.

---

## WebMCP Science Lab

For the **OpenAI WebMCP Challenge 2026**, Esbiko has been meaningfully extended with client-side WebMCP tools that let an AI agent discover and operate the real Doppler Effect simulation alongside the human user.

This is not DOM-click automation and it is not an embedded chatbot. Esbiko registers semantic tools with `document.modelContext.registerTool(...)`; those tools call a validated Doppler adapter and update the same React state used by the visible controls and simulation.
...
The agent can configure source motion, speed, position, emitted frequency, observer position and velocity, and exact-frequency sound type. Read results are calculated from Esbiko's existing Doppler physics logic rather than generated by an LLM.

🌐 **Direct WebMCP demo:** [https://www.esbiko.com/experiments/physics.acoustics.doppler/run](https://www.esbiko.com/experiments/physics.acoustics.doppler/run)

📘 **Implementation, testing, and hackathon evidence:** [`docs/hackathon/webmcp/`](docs/hackathon/webmcp/README.md)
